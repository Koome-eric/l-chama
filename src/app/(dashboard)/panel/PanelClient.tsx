...
const STATUS_LABEL: Record<LoanRequestData['status'], string> = {
  ...
  PENDING_ADMIN: 'Awaiting owner approval',
  REJECTED: 'Rejected',
  ACTIVE: 'Active',
  REPAID: 'Repaid',
};
...
function LoanRequestsTab({ team, currentUserId }: { team: TeamData; currentUserId: string }) {
  ...
  const handleRequest = () => {
    ...
    startTransition(async () => {
      try {
        ...
        toast({ title: 'Loan request submitted', description: 'It needs 2 guarantors before the owner can review it.' });
  ...
  const handleDecision = (id: string, decision: 'APPROVE' | 'REJECT') => {
    ...
    startTransition(async () => {
      try {
        await decideLoanRequest(id, decision, weeks);
          ...
          <Dialog open={requestOpen} onOpenChange={setRequestOpen}>
            ...
            <DialogContent className="max-w-sm">
              <DialogHeader>
                ...
                <DialogDescription>Two chama members will need to guarantee this before the owner reviews it.</DialogDescription>
              </DialogHeader>
